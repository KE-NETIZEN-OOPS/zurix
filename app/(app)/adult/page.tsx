              ...
              <div className="flex items-start justify-between mt-3">
                <div>
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {active.tags?.map(t => <span key={t} className="text-xs bg-zinc-800 text-zinc-400 px-2 py-0.5 rounded-full">{t}</span>)}
                  </div>
                </div>
                <button onClick={() => setActive(null)} className="text-zinc-400 hover:text-white ml-4 text-sm">✕ Close</button>
              </div>
            </div>
          )}
            ...
            {videos.map(v => (
              <button key={v.id} onClick={() => setActive(v)} className="text-left group">
                <div className="relative aspect-video rounded-lg overflow-hidden bg-zinc-800">
                  ...
                  <div className="absolute bottom-1 right-1 bg-black/70 text-white text-xs px-1 rounded">{formatDuration(v.duration_secs ?? 0)}</div>
                </div>
                <p className="text-sm text-zinc-300 mt-1 line-clamp-2 group-hover:text-white transition">{v.title}</p>
              </button>
            ))}
            {videos.length === 0 && <p className="col-span-full text-zinc-500 text-center py-12">No videos curated yet.</p>}
          </div>
        </div>
      </TierGate>
    </AgeGate>
