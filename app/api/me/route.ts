...
export async function GET() {
  ...
  const { data } = await supabase.from('users').select('id, username, display_name, avatar_url, tier, theme, bio, gender, age, location_city, interests, looking_for, hide_data, phone, is_verified').eq('id', user.id).single()
